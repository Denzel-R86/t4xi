...
export type QuoteApplicationResult = { status: number; payload: Record<string, unknown>; snapshot?: PriceSnapshot };
export type QuoteApplicationDependencies = { calculate?: typeof calculateBookingPrice; persist?: typeof persistPriceSnapshot };
const PUBLIC_QUOTE_VEHICLE_CLASS = DEFAULT_VEHICLE_CLASS;
const MAX_ADDRESS_CHARS = 300;
function json(status: number, payload: Record<string, unknown>): QuoteApplicationResult { return { status, payload }; }
function badRequest(message: string) { return json(400, { available: false, error: "invalid_input", message }); }
...
/** Shared application contract used by HTTP and command adapters. */
export async function quoteTrip(body: Record<string, unknown>, deps: QuoteApplicationDependencies = {}): Promise<QuoteApplicationResult> {
  const calculate = deps.calculate ?? calculateBookingPrice;
  const persist = deps.persist ?? persistPriceSnapshot;
  ...
  try {
    const computed = await calculate(input);
