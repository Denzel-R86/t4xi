...
export function renderBookingConfirmationPdf(data: BookingEmailData): Uint8Array {
  ...
  const labels = locale === "en"
    ? { title: "Booking request", ref: "Reference", route: "Route", date: "Date and time", people: "Passengers", luggage: "Luggage", price: "Price", note: "This document confirms receipt of your request. It is not an invoice." }
    : { title: "Boekingsaanvraag", ref: "Referentie", route: "Route", date: "Datum en tijd", people: "Passagiers", luggage: "Bagage", price: "Prijs", note: "Dit document bevestigt de ontvangst van uw aanvraag. Het is geen factuur." };
  ...
  return createSinglePagePdf({
    ...
    lines: [
      ...
      { text: labels.people, x: 48, y: 354, size: 10, color: "#5F666D" },
      { text: String(data.persons), x: 210, y: 354, size: 11, bold: true },
      { text: labels.luggage, x: 48, y: 322, size: 10, color: "#5F666D" },
      { text: data.luggage ?? "-", x: 210, y: 322, size: 11, bold: true },
      { text: labels.price, x: 48, y: 270, size: 10, color: "#5F666D" },
      { text: price, x: 210, y: 266, size: 18, bold: true },
      { text: labels.note, x: 48, y: 188, size: 10, color: "#5F666D" },
