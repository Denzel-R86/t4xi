-- ─────────────────────────────────────────────────────────────────────────────
-- URGENTE SECURITY-HOTFIX voor public.create_booking() — zelfstandig, los van
-- feature/distance-tariff-fallback. Puur hardening; GEEN functionele wijziging.
--
...
--   indienen en een boeking met een willekeurige prijs aanmaken (DEFINER omzeilt RLS).
--
-- AANROEPPLAATSEN (geverifieerd in de codebase): uitsluitend server-side
--   service-role code — app/api/bookings/route.ts via de service-role client.
--   GEEN client/anon-aanroep. Daarom is execute-intrekking van anon/authenticated
--   veilig en breekt de normale serverroute niet.
--
-- HARDENING:
--   1. `SET search_path = ''`. pg_catalog blijft altijd impliciet eerst → built-in
--      functies/types/operatoren zijn onshadowbaar en hoeven niet gekwalificeerd.
--      Het enige user-object (public.bookings) is expliciet gekwalificeerd.
--      (COALESCE/NULLIF/GREATEST/TRIM/CURRENT_DATE zijn SQL-keyword-expressies, geen
--      pg_catalog-functies, en blijven ongekwalificeerd.)
--   2. REVOKE EXECUTE van PUBLIC, anon, authenticated; GRANT alleen aan service_role.
--
-- De functie-BODY is verbatim t.o.v. 20260720090000, met als enige verschil de
-- schema-kwalificatie van public.bookings en de lege search_path.
-- ─────────────────────────────────────────────────────────────────────────────
