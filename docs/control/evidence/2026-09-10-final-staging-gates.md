...
### Merge preparation — head moved after these measurements

The gate figures above were measured on `0f8cae9`, on base `a850ba5`. For the pull request the
branch was rebased onto `main` `0c2caae`. The content is unchanged: the
cumulative patch-id is identical before and after, and the migration SHA-256 is still
`f0b6e57f4c5efae26015a471413a774929dbe99173bcabc04ff31d7cae677825`. Gates were re-run on the
rebased head; see the pull request. Older hashes in this file are kept as the historical record
of when each measurement was taken.
